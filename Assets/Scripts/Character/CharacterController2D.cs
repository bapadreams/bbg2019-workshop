...
namespace Gamekit2D
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            //Debug.Log("position before  " + m_PreviousPosition.ToString() + " ,  Position after " + m_CurrentPosition.ToString());
        ...
        public void Move(Vector2 movement)
        {
            ...
            //Debug.Log("NextMovement " + m_NextMovement.ToString());
        }
        ...
        public void CheckCapsuleEndCollisions(bool bottom = true)
        {
            ...
            else
            {
                ...
                raycastDistance = m_Capsule.size.y * 0.5f + groundedRaycastDistance * 2f; // was size.x
                raycastDistance = groundedRaycastDistance * 2f;
                Debug.DrawLine(new Vector3(raycastStart.x, raycastStart.y) - new Vector3(-.3f, 0, 0), new Vector3(raycastStart.x, raycastStart.y) - new Vector3(+.3f, 0, 0), Color.green);

                if (bottom)
                {
                    raycastDirection = Vector2.down;
                    //Vector2 raycastStartBottomCentre = raycastStart + Vector2.down * (m_Capsule.size.y * 0.5f - m_Capsule.size.x * 0.5f);
                    Vector2 raycastStartBottomCentre = raycastStart + Vector2.down * (m_Capsule.size.y * 0.5f);
                    Debug.DrawLine(new Vector3(raycastStartBottomCentre.x, raycastStartBottomCentre.y) - new Vector3(-.3f, 0, 0), new Vector3(raycastStartBottomCentre.x, raycastStartBottomCentre.y) - new Vector3(+.3f, 0, 0), Color.blue);
                ...
                {
                    raycastDirection = Vector2.up;
                    // the original calc, wasn't working :(, it was based on the ration of the collider, it doesn't work with our colliders
                    Vector2 raycastStartTopCentre = raycastStart;//  + Vector2.up * (m_Capsule.size.y * 0.5f);  //- m_Capsule.size.x * 0.5f);
                    Debug.DrawLine(new Vector3(raycastStartTopCentre.x, raycastStartTopCentre.y) - new Vector3(-.3f, 0, 0), new Vector3(raycastStartTopCentre.x, raycastStartTopCentre.y) - new Vector3(+.3f, 0, 0), Color.red);
                    Debug.DrawLine(new Vector3(raycastStartTopCentre.x, raycastStartTopCentre.y + raycastDistance) - new Vector3(-.3f, 0, 0), new Vector3(raycastStartTopCentre.x, raycastStartTopCentre.y + raycastDistance) - new Vector3(+.3f, 0, 0), Color.yellow);
